...
from utils.collection_config import collections, categories
...
class VectorstorePool:
    def __init__(self, pool_size, collection=""):
        self.pool_size = pool_size
        self.objects = (
            [new_vectorstore(collections[collection]) for i in range(pool_size)]
            if collection
...
pool_size = int(os.environ.get("LLM_POSTGRES_POOL_SIZE", "5"))
vectorstore_pool = VectorstorePool(pool_size)
int_vectorstore_pool = VectorstorePool(pool_size, "internal")
categorised_vecstore_pool = {}
for category in collections:
    categorised_vecstore_pool[category] = VectorstorePool(pool_size, category)
...
async def add_document(
    category, category_folder_id, title, text, html_data, fd_article_id
):
    global pool
    global vectorstore_pool
    global int_vectorstore_pool
    global categorised_vecstore_pool
    try:
        async with pool.acquire() as conn:
            async with conn.transaction():
                ...
                document_chunks = list(
                    map(
                        lambda text_chunk: Document(
                            ...
                            metadata={
                                "source": source_id,
                                "fd_article": "https://lightmetrics.freshdesk.com/a/solutions/articles/"
                                + str(fd_article_id),
                            },
                        ),
                        text_chunks,
                ...
                category_key = "_".join(category.lower().split(" "))
                if category_key in categories.keys():
                    vectorstore = await categorised_vecstore_pool[
                        category_key
                    ].get_object()
                    asyncio.get_running_loop().run_in_executor(
                        executor, lambda: vectorstore.add_documents(document_chunks)
                    )
                    categorised_vecstore_pool[category_key].release_object(vectorstore)
                elif category_key == "internal_articles":
                    vectorstore = await int_vectorstore_pool[category_key].get_object()
                    asyncio.get_running_loop().run_in_executor(
                        executor, lambda: vectorstore.add_documents(document_chunks)
                    )
                    int_vectorstore_pool[category_key].release_object(vectorstore)
                else:
